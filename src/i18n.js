const translations = {
  tr: {
    ...
    modalPacWarningDesc: 'S2shenDPI kapatıldıktan sonra YouTube vb. uygulamalarda internet sorunu yaşarsanız (eski önbellek nedeniyle), Wi-Fi bağlantısını kapatıp açmanız yeterlidir.',
    modalManualWarningTitle: 'DİKKAT:',
    modalManualWarningDesc: 'S2shenDPI kapatıldığında internet bağlantısı tamamen kesilir. İnternete tekrar girmek için cihazınızın Wi-Fi ayarlarından Proxy\'i eski haline (Yok) getirmelisiniz.',
    ...
    adminDesc: 'S2shenDPI\'ın düzgün çalışması için yönetici olarak çalıştırılması gereklidir.',
    ...
    logEngineStarting: (port) => `S2shen Motoru başlatılıyor (Port: ${port})...`,
    ...
    logEngineStopped: (code) => `S2shen motoru beklenmedik şekilde durduruldu (Kod: ${code})`,
    ...
    logReasonFirewall: 'Firewall/Antivirüs S2shenDPI\'ı engelliyor olabilir',
    ...
    logEngineStoppedGrace: 'S2shen motoru kapatıldı.',
    ...
    logEngineActive: '✓ S2shen motoru aktif',
    ...
    logWpcapMissing: 'SpoofDPI başlatılamadı. Proxy binary\'nin yürütme iznine sahip olduğundan emin olun.',
    logAntivirusWarning: 'Güvenlik yazılımınız \'s2shen-proxy\' dosyasını engellemiş olabilir. Lütfen uygulamanın çalışmasına izin verin.',
    ...
    autoStartDesc: 'Sistem açılınca S2shenDPI\'ı başlat',
    ...
    modeDiscordSplit: 'Discord Split',
    modeDiscordSplitDesc: 'Yalnızca Discord trafiği bypass edilir, proxy hiç devreye girmez — oyunlarda sıfır ek gecikme.',
    discordSplitActiveNotice: 'Discord Split aktif: ISS rehberi, mod (Turbo/Dengeli/Güçlü) ve chunk size ayarları bu modda kullanılmaz — yalnızca Discord bypass edilir.',
    modeRequiresNpcap: 'Kernel motoru gereklidir',
    modeBadgeSmooth: 'Akıcı',
    modeBadgeGame: 'Oyun',
    modeBadgeSuper: 'Süper',

    // Gelişmiş (Kernel Motor)
    ...
    advancedNpcapInstalled: 'Kernel Motoru Hazır — Gelişmiş özellikler aktif',
    advancedNpcapMissing: 'Kernel motoru bulunamadı — Güçlü mod kısıtlı çalışır',
    advancedNpcapInstallBtn: 'KERNEL MODÜLÜNÜ YÜKLE',
    advancedNpcapWhy: 'Linux kernel\'ının NFQUEUE modülü ağ paketlerine düşük seviyede erişim sağlar. Sahte paket (fake packet) gönderme gibi gelişmiş DPI aşma teknikleri kullanılabilir.',
    ...
    npcapRestartWarning: 'Kernel modülünün aktif olması için sistemi yeniden başlatmanız gerekebilir.',
    logStrongFake: '🚀 Güçlü Mod: Fake Paket (3) aktif.',
    logStrongNoDriver: '⚠️ Güçlü Mod: Kernel motoru yok, sadece Chunk-1 aktif.',
    logStrongChunkOnly: '🛡️ Güçlü Mod: Chunk-1 aktif.',
    logNpcapFallback: '⚠️ Kernel motoru yanıt vermiyor. Gelişmiş bypass kapatılıp tekrar deneniyor...',
    advancedNpcapHint: 'Daha güçlü bypass için kernel modülü gerekli',
    ...
    devRole: 'S2shenDPI Geliştiricisi',
    devSubscribe: 'Discord',
    devSupport: 'Github',

    sectionNotice: 'ÖNEMLİ BİLGİ',
    noticeTitle: 'Güvenlik ve Yanlış Pozitif',
    noticeDesc: 'S2shen motoru, Windows Defender AI gibi yapay zeka tabanlı sistemler tarafından bazen "yanlış pozitif" olarak algılanabilir. Bu durum tamamen zararsızdır. Ayrıca Kaspersky, ESET gibi yazılımlar HTTPS tarama özelliğiyle bağlantıyı engelleyebilir. Erişim sorunu yaşarsanız bu ayarları kontrol edin.',

    // Dialogs
    confirmExitTitle: 'Çıkış',
    confirmExitDesc: 'S2shen motorunu durdurup çıkmak istediğinize emin misiniz?',
    ...
    domainInvalid: 'Geçersiz domain. Yalnızca harf, rakam, tire ve nokta kullanın (örn: *.example.com).',
  ...
  en: {
    ...
    modalPacWarningDesc: 'If apps like YouTube lose internet access after closing S2shenDPI (due to cached connections), simply toggle your device\'s Wi-Fi off and on.',
    modalManualWarningTitle: 'ATTENTION:',
    modalManualWarningDesc: 'When S2shenDPI is closed, your device will completely lose internet access. To restore connection, you must remove the Proxy setting from your Wi-Fi settings.',
    ...
    adminDesc: 'S2shenDPI needs to run as administrator to work correctly.',
    ...
    logEngineStarting: (port) => `S2shen Engine starting (Port: ${port})...`,
    ...
    logEngineStopped: (code) => `S2shen engine stopped unexpectedly (Code: ${code})`,
    ...
    logReasonFirewall: 'Firewall/Antivirus may be blocking S2shenDPI',
    ...
    logEngineStoppedGrace: 'S2shen engine stopped.',
    ...
    logEngineActive: '✓ S2shen engine active',
    ...
    logWpcapMissing: 'SpoofDPI could not start. Make sure the proxy binary has execute permission.',
    logAntivirusWarning: 'Your security software may have blocked \'s2shen-proxy\'. Please allow the application to run.',
    ...
    autoStartDesc: 'Launch S2shenDPI at system startup',
    ...
    modeDiscordSplit: 'Discord Split',
    modeDiscordSplitDesc: 'Only Discord traffic is bypassed, proxy is never used — zero added latency in games.',
    discordSplitActiveNotice: 'Discord Split is active: the ISP guide, mode (Turbo/Balanced/Strong) and chunk size settings are not used in this mode — only Discord is bypassed.',
    modeRequiresNpcap: 'Kernel engine required',
    modeBadgeSmooth: 'Smooth',
    modeBadgeGame: 'Game',
    modeBadgeSuper: 'Super',

    // Advanced (Kernel engine)
    ...
    advancedNpcapInstalled: 'Kernel Engine Ready — Advanced features active',
    advancedNpcapMissing: 'Kernel engine not found — Strong mode limited',
    advancedNpcapInstallBtn: 'LOAD KERNEL MODULE',
    advancedNpcapWhy: 'The Linux kernel NFQUEUE module provides low-level access to network packets, enabling advanced DPI bypass techniques like fake packet injection.',
    ...
    npcapRestartWarning: 'You may need to restart for the kernel module to activate.',
    logStrongFake: '🚀 Strong Mode: Fake Packet (3) active.',
    logStrongNoDriver: '⚠️ Strong Mode: No kernel engine, Chunk-1 only.',
    logStrongChunkOnly: '🛡️ Strong Mode: Chunk-1 active.',
    logNpcapFallback: '⚠️ Kernel engine not responding. Disabling advanced bypass and retrying...',
    advancedNpcapHint: 'Kernel engine required for stronger bypass',
    ...
    devRole: 'S2shenDPI Developer',
    devSubscribe: 'Discord',
    devSupport: 'Github',

    sectionNotice: 'IMPORTANT',
    noticeTitle: 'Security & False Positives',
    noticeDesc: 'The S2shen engine may sometimes be flagged as a "false positive" by AI-based systems like Windows Defender. This is completely harmless. Also, antivirus software like Kaspersky or ESET may block connections with their HTTPS scanning. If you experience issues, check those settings.',

    // Dialogs
    confirmExitTitle: 'Exit',
    confirmExitDesc: 'Are you sure you want to stop the S2shen engine and exit?',
    ...
    domainInvalid: 'Invalid domain. Use only letters, digits, hyphens and dots (e.g. *.example.com).',
